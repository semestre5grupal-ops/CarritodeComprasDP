  ...
  <main id="contenido" aria-label="Contenido principal">
    ...
    <section id="hero" class="hero wrap-wide" aria-labelledby="hero-title" aria-describedby="hero-summary">
        ...
        <div class="hero-carousel" aria-hidden="true" id="hero-carousel">
            <img v-for="(slide, index) in slides" :key="index" :src="slide" alt="" class="carousel-slide" :class="{ active: activeSlide === index }">
            <div class="carousel-overlay"></div>
        </div>
        
        <!-- Carousel Controls -->
        <div class="carousel-controls" aria-label="Controles del carrusel">
            <button type="button" @click="prevSlide" id="btn-prev-slide" class="carousel-btn" aria-label="Imagen anterior" aria-controls="hero-carousel">
            ...
            <button type="button" @click="nextSlide" id="btn-next-slide" class="carousel-btn" aria-label="Siguiente imagen" aria-controls="hero-carousel">
    ...
    <section id="catalogo" class="catalog-section wrap-wide" aria-labelledby="sec-productos" aria-live="polite">
        ...
        <ul id="product-grid" class="product-grid" aria-label="Listado de productos destacados" :aria-busy="loading">
            <li v-if="loading" class="loading" aria-live="polite">Cargando productos...</li>
            <li v-else-if="error">Error: {{ error }}</li>
            <ProductCard v-else v-for="p in filteredProducts" :key="p.id" :product="p" @add-to-cart="handleAddToCart" />
        </ul>
    </section>
    ...
    <section class="discount-cta wrap-wide" aria-labelledby="sec-descuento">
        <div class="discount-cta__content">
            ...
            <RouterLink to="/register" class="btn discount-cta__btn">Crear cuenta</RouterLink>
        </div>
    </section>
